...
$response = ""; 
	...
	exit();
		...
		$countCheck = mysql_query("SELECT id FROM questions_k")or die(mysql_error());
	...
	VALUES ('$username', '$percent', now())")or die(mysql_error());
		...
		$ques = mysql_query("SELECT * FROM questions_k")or die(mysql_error());
		echo '<h3>SOLUTIONS FOR THE QUIZ </h3>';
		while($row = mysql_fetch_array($ques))
		{
		$temp = $row['question'];
		$id = $row['question_id'];
		echo '<h4>Q.'.$id.'. '.$temp.'</h4>';
		$ans = mysql_query("SELECT * FROM answers_k WHERE question_id=$id AND correct = '1'")or die(mysql_error());
