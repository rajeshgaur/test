...
xhr.send(vars);
		...
		document.getElementById('btnSpan').innerHTML += '<a href="quiz_kin.php?question=<?php echo $next; ?>">Click here now</a>';
...
hr.open("GET", "questions_kin.php?question=" + <?php echo $question; ?>, true);
  hr.send();
}
function x() {
...
function post_answer(){
	var p = new XMLHttpRequest();
		...
		if(p.readyState == 4 && p.status == 200) {
			...
			var url = 'quiz_kin.php?question=<?php echo $next; ?>';
			window.location = url;
	}
}
p.send(vars);
