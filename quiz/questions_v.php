...
if(isset($_GET['question'])){
	...
	$answers = "";
	$q = "";
	$sql = mysql_query("SELECT question_id FROM questions_v");
	...
	if($arrCount >= $numQuestions){
		...
		<p>There are no more questions. Please enter your first and last name and click next</p>
				<form action="userAnswers_v.php" method="post">
	...
	$singleSQL = mysql_query("SELECT * FROM questions_v WHERE id='$question' LIMIT 1");
		while($row = mysql_fetch_array($singleSQL)){
			...
			$sql2 = mysql_query("SELECT * FROM answers_v WHERE question_id='$question' ORDER BY rand()");
